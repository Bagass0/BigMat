...
if (($_SERVER['REQUEST_METHOD'] == 'POST')){
    ...
    $TXT_ACCUEIL_ST = $fct->cleanXSS($_POST['TXT_ACCUEIL_ST']);
    $TXT_ACCUEIL_T_EDITO = $fct->cleanXSS($_POST['TXT_ACCUEIL_T_EDITO']);
    $TXT_ACCUEIL_EDITO = $fct->cleanXSS($_POST['TXT_ACCUEIL_EDITO']);
    $TITRE_BLOCS = $fct->cleanXSS($_POST['TITRE_BLOCS']);
    ...
    for ($i = 1; $i <= $NB_SLIDES; $i++) {
        ...
        if (!(in_array($extensionFichier, $extensionsAutorisees))) {
            ...
            if (move_uploaded_file($_FILES[$namepicture]["tmp_name"],$repertoireDestination.$nomDestination)) {
                ...
                $req = mysqli_query($db, $sql) or die('Erreur 500-1, Merci de contacter l\'administrateur.');
...
$sql = 'SELECT * FROM `SITE_ACCUEIL` WHERE EVENT_ID = "'.$event.'"';
$req = mysqli_query($db, $sql) or die('Erreur 500-1, Merci de contacter l\'administrateur.');
