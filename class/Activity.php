...
class Activity extends Model {
    ...
    public function add ($THEME, $DESCRIPTION, $DATE, $HEURE, $SALLE, $event) {
        ...
        $req = mysqli_query($this->mysqli, $sql) or die('Erreur 500-C1, Merci de contacter l\'administrateur.');
	...
	public function updateActivity($theme, $description, $date, $heure, $salle, $id){
		$sql = "UPDATE `ACTIVITES` 
			    ...
			    WHERE `ID` = '".$id."';";
		$req = mysqli_query($this->mysqli, $sql) or die('Erreur 500-C2, Merci de contacter l\'administrateur.');
		$data = mysqli_fetch_array($req);
	}

	public function selectInfosListe($condition)
	{
		$sql = 'SELECT * FROM ACTIVITES WHERE ' . $condition . ' ORDER BY ID ASC';
		$req = mysqli_query($this->mysqli, $sql) or die('Erreur 500-C3, Merci de contacter l\'administrateur.');
