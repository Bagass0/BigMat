...
class sProgramme extends Sites{
	...
	public function updateBanniere($nomDestination, $event){
		...
		$req = mysqli_query($this->mysqli, $sql) or die('Erreur 500-M1, Merci de contacter l\'administrateur.');
		return mysqli_fetch_array($this->mysqli, $req);
	}

	public function setFileBloc($i, $nomDestination, $idEvent){
		$sql = "UPDATE `SITE_PROGRAMME` SET `PIC_PROGRAMME_J" . $i . "` = '" . mysqli_real_escape_string($this->mysqli, $nomDestination) . "' WHERE `EVENT_ID` = '$idEvent'";
		$req = mysqli_query($this->mysqli, $sql) or die('Erreur 500-M2, Merci de contacter l\'administrateur.');
	}
	...
	public function updateContent($TXT_PROGRAMME_ST, $NB_PROGRAMME, $event){
		...
		$req = mysqli_query($this->mysqli, $sql) or die('Erreur 500-M3, Merci de contacter l\'administrateur.');
		return mysqli_fetch_array($this->mysqli, $req);
