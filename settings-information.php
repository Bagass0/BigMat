...
if (($_SERVER['REQUEST_METHOD'] == 'POST') && isset($_POST['valider'])){
    ...
    $TXT_INFOS_ST = $fct->cleanXSS($_POST['TXT_INFOS_ST']);
    $NB_INFOS = $fct->cleanXSS($_POST['NB_INFOS']);
    ...
    for ($i = 1; $i <= $NB_INFOS; $i++) {
        ...
        $TXT_INFOS_TITRE_CONTENT = $fct->cleanXSS($_POST['TXT_INFOS_P'.$i.'_TITRE']);
        $TXT_INFOS = "TXT_INFOS_P".$i;
        $TXT_INFOS_CONTENT = $fct->cleanXSS($_POST['TXT_INFOS_P'.$i]);
        $ICO_INFOS_P = "ICO_INFOS_P".$i;
        $ICO_INFOS_P_CONTENT = $fct->cleanXSS($_POST['ICO_INFOS_P'.$i]);
							...
							<table style="margin-top:30px;">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="text-align: right;"><a data-fancybox data-type="iframe" data-src="information.php?event=<?php echo $event; ?>&view=admin" href="javascript:;" style="text-decoration:none;"><i class="fas fa-eye"></i></a></th>
                                    </tr>
                                </thead>
                            </table>
                            
                            <form method="post" action="settings-information.php?event=<?php echo $event; ?>" enctype="multipart/form-data">
                                <div class="row uniform">
                                    <div class="12u$ 12u$(xsmall)">
                                        <div class="fakeinput">Bannière :
                                            ...
                                            <?php if($data["PIC_INFOS"] != ""){
                                                echo'
                                                <a href="delete.php?picture='.$data['PIC_INFOS'].'&url=settings-information.php&event='.$event.'&jour=PIC_INFOS&table=information" class="confirm" style="text-decoration:none; float:right; margin-left: 10px;" title="Supprimer">
